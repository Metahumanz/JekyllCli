...
发布文章或博客设置时，可以让 AI 生成更有意义的 Git 提交信息。

配置步骤：

1. 选择内置提供商，或选择 `自定义`
2. 填写 `Base URL` 和 `API Key`
3. 点击 `刷新模型`，从完整下拉列表选择模型；也可以直接手填模型标识
4. 如果服务的模型列表地址不是默认的 `<Base URL>/models`，填写可选的 `Models URL`
5. 点击 `测试生成` 验证配置，再保存档案

说明：

- API Key 使用当前 Windows 用户的 DPAPI 加密后保存，界面默认隐藏
- 可以创建多套档案，快速切换不同提供商或本地服务
- 文章发布和博客设置发布的 AI 开关彼此独立
- 发布方式可以选择 `每次确认并可编辑` 或 `直接提交`
- AI 请求只发送限长的改动摘要；二进制文件只发送文件名
- AI 调用失败时，软件会询问是否使用默认固定文案继续发布
